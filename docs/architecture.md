...
- `finance` / `resources`: money, accounts, ledger movements, transfers, recurrence generation, purchases, cycles, invoice settlement and credit carry-forward.
...
- Habit history is interpreted against the schedule valid on that date; today's incomplete target does not break the current streak before the day ends.
